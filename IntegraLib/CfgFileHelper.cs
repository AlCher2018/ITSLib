...
namespace IntegraLib
{
    ...
    {
        #region config files
        public static string GetAppConfigFile(string appCfgFileName = null)
        {
            string fullName = AppEnvironment.GetFullSpecialFileNameInAppDir("config", appCfgFileName, true);

            if (!System.IO.File.Exists(fullName)) fullName = null;

            return fullName;
        }

        public static string GetAppSettingsConfigFile(string cfgFileName = "AppSettings")
        {
            return GetAppConfigFile(cfgFileName);
        }

        public static string GetDBConnectionsConfigFile(string cfgFileName = "DBConnection")
        {
            return GetAppConfigFile(cfgFileName);
        }
        #endregion
        ...
        #region настройки из config-файла
        // Returns:
        ...
        public static string GetDBConnectionStringFromConfigFile(string connectionName = null)
        {
            string retVal = null;
            try
            {
                if (connectionName.IsNull())
                {
                    ...
                    {
                        if (item.Name != "LocalSqlServer") { connectionName = item.Name; break; }
                    }
                }
                retVal = ConfigurationManager.ConnectionStrings[connectionName].ConnectionString;
            }
            catch (Exception ex)
            {
                retVal = string.Format("Error in cfg connection string name {0}: {1}", connectionName, ex.Message);
            }
